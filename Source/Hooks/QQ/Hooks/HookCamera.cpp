...
)
{
    ...
    GetWindowRect(GetDesktopWindow(), &scaledDesktop);

    cx = scaledDesktop.right - scaledDesktop.left;
    cy = scaledDesktop.bottom - scaledDesktop.top;
...
BOOL IsGdiScaled()
{
    ...
    if (desktopBmp.bmWidth == desktop.right - desktop.left && desktopBmp.bmHeight == desktop.bottom - desktop.top)
        return FALSE;
